...
test("Sandwich REAL: Top branch + Bottom-focused branch", async ({
  browser,
}) => {
  ...
  try {
    await Promise.all([
      ...
      (async () => {
        ...
        await expect(bottomPage.locator(".shopping_cart_badge")).toHaveText(
          "1",
        );
        ...
        await expect(bottomPage.locator(".inventory_item_name")).toHaveText(
          "Sauce Labs Backpack",
        );
      })(),
    ]);
  } finally {
    await Promise.all([topContext.close(), bottomContext.close()]);
  }
});
